---
#
# .-'_.---._'-.
# ||####|(__)||   Protect your secrets, protect your business.
#   \\()|##//       Secure your sensitive data with Aegis.
#    \\ |#//                    <aegis.ist>
#     .\_/.
#

layout: default
keywords: Aegis, toc, table of contents
title: Contents Outline
description: navigation map to everything
micro_nav: true
page_nav:
  prev:
    content: local development
    url: '/docs/contributing'
  next:
    content: <strong>Aegis</strong> Sentinel CLI
    url: '/docs/sentinel'
---

Here’s your handy guide to everything on this site. 

We have sorted the links in the order of importance and priority for you.

* [Home Page](/)
* [**Aegis** Deep Dive](/docs/architecture)
* [**Aegis** Design Decisions](/docs/philosophy)
* [Quickstart Guide](/docs/)
* [Registering Secrets](/docs/register)
  * [Overview and Prerequisites](/docs/use-case-overview)
  * [Using **Aegis Sidecar**](/docs/use-case-sidecar)
  * [Using **Aegis SDK**](/docs/use-case-sdk)
  * [Using **Aegis Init Container**](/docs/use-case-init-container)
  * [Encrypting Secrets](/docs/use-case-encrypt)
  * [Transforming Secrets](/docs/use-case-transform)
  * [Showcase of Everything so Far](/docs/use-case-showcase)
* [Local Development](/docs/contributing)
* [Configuring **Aegis**](/docs/configuration)
* [**Aegis** Production Deployment](/production)
* [**Aegis SDK** Documentation](/docs/sdk)
* [**Aegis Sentinel** Manual](/docs/sentinel)
* [**Aegis** Changelog](/changelog)
* [What’s Coming Up Next](/timeline)
* [How to Release a New Version](/release)
* [Miscellaneous](/misc)
* [Privacy Policy](/privacy)
* [How to Pronounce “*Aegis*”](/pronunciation)
* [Media Kit](/media)
* [Maintainers](/maintainers)
* [Contact](/contact)
